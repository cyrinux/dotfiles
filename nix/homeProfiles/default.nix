{ pkgs, ... }:
{
  ...
  programs.ssh = {
    ...
    # extraConfig = lib.strings.removeSuffix "\n" (builtins.readFile config.age.secrets.ssh_config.path);
    includes = [ "~/.dotfiles-private/.ssh/config" ];
  ...
  systemd.user.sessionVariables = {
    ...
    PATH = ''$HOME/.local/bin:$HOME/.dotfiles-private/.local/bin:$PATH'';
    ...
    MBSYNC_CONFIG = ''$HOME/.config/mbsync/config'';
  ...
  programs.kitty = {
    enable = true;
    keybindings = {
      "ctrl+f>1" = "set_font_size 14";
      "ctrl+f>2" = "set_font_size 20";
      "kitty_mod+n" = "new_os_window_with_cwd cglaunch kitty --detach";
      "kitty_mod+0" = "change_font_size all 0";
      "kitty_mod+l" = "kitten hints --type url";
      "kitty_mod+p" = "kitten hints --type path";
      "kitty_mod+y" = "kitten password.py";
      "kitty_mod+g" = "pipe @ansi_alternate_scrollback os_window less +G -R";
      "kitty_mod+b" = "launch --type overlay --stdin-source=@screen_scrollback kak -e 'hook -once buffer BufReadFifo .* %{ eval -client client0 exec gj }'";
    };
