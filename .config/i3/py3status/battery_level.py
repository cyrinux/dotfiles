...
class Py3status:
        ...
        deprecated = {
            ...
            'substitute_by_value': [
                ...
                },
                {
                    ...
                    'msg':
                    'obsolete parameter use `format = "{icon} {percent}%"`',
                },
            ],
        }
    ...
    def _extract_battery_information_from_acpi(self):
        ...
        acpi_list = [
            acpi_list[i:i + 2] for i in range(0,
                                              len(acpi_list) - 1, 2)
        ]
    ...
    def _extract_battery_information_from_sys(self):
        ...
        for path in iglob(os.path.join(self.sys_battery_path, "BAT*")):
            ...
            remaining_energy = r.get("POWER_SUPPLY_ENERGY_NOW",
                                     r.get("POWER_SUPPLY_CHARGE_NOW"))
            ...
            if "Full" in r["POWER_SUPPLY_STATUS"]:
                battery["time_remaining"] = FULLY_CHARGED
    ...
    def _refresh_battery_info(self):
        ...
        if type(self.battery_id) == int:
            ...
            total_capacity = sum(
                [battery['capacity'] for battery in battery_list])
            ...
            self.percent_charged = int(
                sum([
                    battery["capacity"] / total_capacity *
                    battery["percent_charged"] for battery in battery_list
                ]))

            self.charging = any(
                [battery["charging"] for battery in battery_list])
